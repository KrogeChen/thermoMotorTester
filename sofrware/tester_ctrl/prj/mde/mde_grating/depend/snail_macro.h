...
#define macro_creat_queueBasce(in_queueName,in_queueBytes) \
        static sdt_int8u  in_queueName##_buff[in_queueBytes];\
	    static sdt_int8u  in_queueName##_bytes = 0;\
		static const sdt_int8u in_queueName##_maxBytes = in_queueBytes;\
		static sdt_int8u in_queueName##_in_idx = 0;\
		static sdt_int8u in_queueName##_out_idx = 0
//-------------------------------------------------------------------------------------------------
//获取队列的字节数
//-------------------------------------------------------------------------------------------------
...
do\
{\
	out_bytes = in_queueName##_bytes;\
}while(0)
//-------------------------------------------------------------------------------------------------
//获取队列full的情况
//-------------------------------------------------------------------------------------------------
#define macro_pull_queueBasce_full(in_queueName,out_full) \
do\
{\
    if(in_queueName##_bytes > in_queueName##_maxBytes)\
	{\
...
//从队列中取一个数据
//-------------------------------------------------------------------------------------------------
#define macro_pull_queueBasce_data(in_queueName,out_data) \
do\
{\
...
//-------------------------------------------------------------------------------------------------
#define macro_push_queueBasce_data(in_queueName,in_data) \
do\
{\
    if(in_queueName##_bytes < in_queueName##_maxBytes)\
	{\
